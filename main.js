...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (typeof gsap !== 'undefined' && typeof ScrollTrigger !== 'undefined') {
    ...
    gsap.set(productCards, { autoAlpha: 0, y: 120, scale: 0.9 });
    ScrollTrigger.batch(productCards, {
      start: "top 95%", // Trigger lower so it always catches on mobile
      onEnter: batch => gsap.to(batch, { autoAlpha: 1, y: 0, scale: 1, stagger: 0.15, ease: "expo.out", duration: 1.4, overwrite: true }),
      onLeaveBack: batch => gsap.to(batch, { autoAlpha: 0, y: 50, ease: "power2.in", overwrite: true })
    });
